...
const validateDateRange = (startDate, endDate = null) => {
  ...
  if (start > today) {
    return { valid: false, error: 'Start date cannot be in the future. Please select a valid date.' };
  }
  if (endDate) {
    const end = normalizeDate(new Date(endDate));
    if (end > today) {
      return { valid: false, error: '⚠️ End date cannot be in the future. Please select a valid date.' };
    }
    if (start > end) {
      return { valid: false, error: '⚠️ Start date cannot be after end date. Please select valid dates.' };
    }
  }
  return { valid: true };
};
...
const getSalesReport = async (req, res) => {
  try {
    ...
    try {
      if (filterType === 'custom') {
        if (!startDate || !endDate) {
          return res.status(400).render('error', { message: 'Custom date range requires both start and end dates' });
        }
    ...
    const dateFilter = { createdOn: { $gte: queryStartDate, $lt: queryEndDate } };
    ...
    const formattedOrders = orders.map(order => {
      const userData = order.user
        ? { _id: order.user._id, name: order.user.name || 'Guest User', email: order.user.email || 'N/A', phone: order.user.phone || order.deliveryAddress?.phone || 'N/A' }
        : { name: order.deliveryAddress?.name || 'Guest User', email: 'N/A', phone: order.deliveryAddress?.phone || 'N/A' };
      ...
      if (order.paymentMethod) {
        switch (order.paymentMethod.toLowerCase()) {
          case 'cod': paymentMethod = 'Cash on Delivery'; break;
          case 'wallet': paymentMethod = 'Wallet'; break;
          case 'razorpay':
          case 'online': paymentMethod = 'Online Payment'; break;
      ...
      return {
        ...
        paymentMethodRaw: order.paymentMethod || 'N/A',
    ...
    res.status(500).render('error', { message: 'Error loading sales report', error });
  }
};
...
const exportSalesReportToExcel = async (req, res) => {
  try {
    ...
    try {
      if (filterType === 'custom') {
        if (!startDate || !endDate) return res.status(400).json({ error: 'Custom date range requires both start and end dates' });
        const validation = validateDateRange(startDate, endDate);
    ...
    const dateFilter = { createdOn: { $gte: queryStartDate, $lt: queryEndDate } };
    const orders = await Order.find(dateFilter).populate({ path: 'user', select: 'name email phone' }).sort({ createdOn: -1 }).lean();
    if (orders.length === 0) return res.status(400).json({ error: 'No data available for export.' });
    ...
    worksheet.columns = [
      { width: 12 }, { width: 18 }, { width: 14 }, { width: 35 }, { width: 10 },
      { width: 14 }, { width: 14 }, { width: 14 }, { width: 14 }, { width: 16 }, { width: 12 }
    ];
    const headers = ['Order ID', 'Customer Name', 'Phone', 'Products', 'Items', 'Total Price', 'Discount', 'Coupon', 'Final Amount', 'Payment Method', 'Status'];
    ...
    orders.forEach(order => {
      ...
      const productsText = order.orderItems?.map(item => {
        let productStr = `${item.productName} (Qty: ${item.quantity})`;
        if (item.variant) productStr += ` - ${item.variant}`;
        return productStr;
      }).join('; ') || 'N/A';
      ...
      const row = worksheet.addRow([
        order.orderId, userData, phone, productsText, items, order.totalPrice,
        order.discount, couponCode, order.finalAmount, paymentMethod, order.status
      ]);
...
const exportSalesReportToPDF = async (req, res) => {
  try {
    ...
    doc.moveDown(0.2);
    doc.fontSize(12).font('Helvetica-Bold').text(displayLabel, { align: 'center' });
    doc.moveDown(0.1);
    doc.fontSize(10).font('Helvetica').text(`Period: ${queryStartDate.toLocaleDateString('en-IN')} to ${new Date(queryEndDate.getTime() - 1).toLocaleDateString('en-IN')}`, { align: 'center' });
    doc.fontSize(9).font('Helvetica-Oblique').text(`Generated on: ${new Date().toLocaleString('en-IN')}`, { align: 'center' });
    doc.moveDown(0.3);
    ...
    doc.text(`Total Sales Amount: ₹${totalRevenue.toLocaleString('en-IN')}`, 40);
    ...
    const col1 = 40, col2 = 110, col3 = 200, col4 = 400, col5 = 450, col6 = 530, col7 = 610, col8 = 710;
    ...
    doc.moveTo(30, tableTop + 12).lineTo(810, tableTop + 12).stroke();
    ...
    orders.forEach(order => {
      ...
      if (order.paymentMethod) {
        switch (order.paymentMethod.toLowerCase()) {
          ...
          case 'wallet': paymentMethod = 'Wallet'; break;
          case 'razorpay':
          case 'online': paymentMethod = 'Online'; break;
          default: paymentMethod = order.paymentMethod.substring(0, 8);
        }
      }
      ...
      doc.text(items.toString(), col4, yPosition, { width: 40, align: 'center' });
      doc.text(`₹${order.finalAmount.toFixed(2)}`, col5, yPosition, { width: 70, align: 'right' });
      doc.text(`₹${order.discount.toFixed(2)}`, col6, yPosition, { width: 70, align: 'right' });
