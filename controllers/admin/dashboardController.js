...
const loadDashboard = async (req, res) => {
  ...
  try {
      ...
      { $unwind: "$orderItems" },
      {
        $group: {
          ...
          activeItems: { 
            $sum: { 
              $cond: [{ 
                $in: ["$orderItems.status", ["Placed", "Confirmed", "Processing", "Shipped", "OutForDelivery"]] 
              }, "$orderItems.quantity", 0] 
            } 
          },
          returnRequestItems: { $sum: { $cond: [{ $eq: ["$orderItems.status", "Return Request"] }, "$orderItems.quantity", 0] } }
        }
      }
    ]);
    ...
    const totalRevenueAgg = await Order.aggregate([
      ...
      { 
        $match: { 
          "orderItems.status": { $nin: ["Cancelled", "Returned", "Return Request", "Payment Failed"] } 
        } 
      },
      { $group: { _id: null, totalRevenue: { $sum: "$orderItems.total" } } }
    ]);
    ...
    const chartAgg = await Order.aggregate([
      { $match: { createdOn: { $gte: startDate, $lte: endDate } } },
      { $unwind: "$orderItems" },
      { $match: { "orderItems.status": { $nin: ["Cancelled", "Returned", "Return Request", "Payment Failed"] } } },
      {
        $group: {
          _id: { $dateToString: { format: chartDateFormat, date: '$createdOn' } },
    ...
    const topProducts = await Order.aggregate([
      { $match: { createdOn: { $gte: startDate, $lte: endDate } } },
      { $unwind: '$orderItems' },
      { $match: { 'orderItems.status': { $nin: ['Cancelled', 'Returned', 'Payment Failed'] } } },
      {
        $group: {
          _id: '$orderItems.productName',
          sold: { $sum: '$orderItems.quantity' },
