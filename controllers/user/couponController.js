...
const formatCouponWithUsage = (coupon, currentUserId = null) => {
  const appliedUsersArray = Array.isArray(coupon.appliedUsers)
    ? coupon.appliedUsers
    : [];
  const usedCount = appliedUsersArray.length;
  const limitNum = Number(coupon.limit) || 0;
  const usagePercentage =
    limitNum > 0 ? Math.round((usedCount / limitNum) * 100) : 0;
  const remainingUses = Math.max(0, limitNum - usedCount);

  const userHasUsed = currentUserId
    ? appliedUsersArray.some(
        (applied) =>
          applied.userId &&
          applied.userId._id.toString() === currentUserId.toString()
      )
    : false;
  ...
  try {
    ...
    const formattedCoupons = coupons.map((coupon) => {
      ...
      return {
        ...
        isUsable:
          status === "active" &&
          !(coupon.usageType === "once" && usageData.userHasUsed),
      };
    });
...
const applyCouponAtCheckout = async (req, res) => {
  try {
    ...
    if (!cartTotal) {
      return res
        .status(400)
        .json({ success: false, message: "Cart total is required" });
    }
    ...
    if (discountPrice <= 0) {
      return res
        .status(400)
        .json({ success: false, message: "This coupon provides no discount" });
    }
    ...
    const minimumPricePaise = Math.round(
      (parseFloat(coupon.minimumPrice) || 0) * 100
    );
    ...
    if (cartTotalPaise < minimumPricePaise) {
      return res.status(400).json({
        success: false,
        message: `Minimum purchase of ₹${(minimumPricePaise / 100).toFixed(
          2
        )} required`,
      });
    }
    ...
    if (totalDiscountPaise <= 0) {
      return res
        ...
        .json({
          success: false,
          message: "No discount applicable for this cart",
        });
    }
    ...
    if (cartItems && Array.isArray(cartItems) && cartItems.length > 0) {
      ...
      if (Math.abs(computedTotalPaise - cartTotalPaise) > 1) {
        return res
          .status(400)
          .json({ success: false, message: "Cart total mismatch" });
      }
      ...
      cartItems.forEach((item, index) => {
        ...
        const discountPaise = Math.round(
          (subtotalPaise * totalDiscountPaise) / cartTotalPaise
        );
      ...
      if (diffPaise !== 0) {
        ...
        updatedCartItems.forEach((item, idx) => {
          if (
            ...
            parseFloat(updatedCartItems[maxSubtotalIdx].subtotal)
          ) {
            maxSubtotalIdx = idx;
          }
        });
        const adjustDiscount =
          parseFloat(updatedCartItems[maxSubtotalIdx].discount) +
          diffPaise / 100;
        const adjustDiscounted =
          parseFloat(updatedCartItems[maxSubtotalIdx].discountedSubtotal) -
          diffPaise / 100;
        updatedCartItems[maxSubtotalIdx].discount = adjustDiscount.toFixed(2);
        updatedCartItems[maxSubtotalIdx].discountedSubtotal =
          adjustDiscounted.toFixed(2);
      }
    } else {
      updatedCartItems = [];
    }
    ...
    if (updatedCartItems.length > 0) {
      console.log(
        "   Updated Items:",
        updatedCartItems.map((i) => `${i.name}: -₹${i.discount}`)
      );
    }
...
const searchCoupons = async (req, res) => {
  try {
    ...
    const coupons = await Coupon.find({
      ...
      .select(
        "couponCode couponName description discountPrice discountType minimumPrice maxDiscountAmount limit activeDate expireDate appliedUsers"
      )
      .populate("appliedUsers.userId", "email")
      .limit(10)
      .lean();
