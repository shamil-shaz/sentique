    ...
    try {
        ...
        cart.items.forEach((item, index) => {
            try {
                ...
                if (!product) {
                    console.warn(` Item ${index}: Missing product`);
                ...
                if (product.isBlocked === true) {
                    console.warn(` Item ${index}: Product blocked - ${product.productName}`);
                    blockedProducts.push({
                ...
                if (product.brand && product.brand.isBlocked === true) {
                    console.warn(`Item ${index}: Brand blocked - ${product.productName}`);
                    blockedProducts.push({
                        name: product.productName,
                ...
                if (product.category && product.category.isListed === false) {
                    console.warn(` Item ${index}: Category not listed - ${product.productName}`);
                    blockedProducts.push({
                        name: product.productName,
        ...
        if (blockedProducts.length > 0) {
            console.log(' Found blocked products:', blockedProducts);
