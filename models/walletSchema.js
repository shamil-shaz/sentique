...
walletSchema.virtual("recentTransactions").get(function () {
  return this.transactions.sort((a, b) => b.date - a.date).slice(0, 10);
});

walletSchema.methods.addCredit = function (
  amount,
  description,
  reason,
  productName = null,
...
walletSchema.methods.addDebit = function (
  amount,
  description,
  reason,
  productName = null,
...
walletSchema.methods.getTransactionsByDateRange = function (
  startDate,
  endDate
) {
...
walletSchema.statics.getUserTransactionHistory = async function (
  userId,
  limit = 20
) {
  const wallet = await this.findOne({ user: userId });
  ...
  return wallet.transactions.sort((a, b) => b.date - a.date).slice(0, limit);
};
