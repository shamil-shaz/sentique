...
// // Variant schema: each variant has size, stock, regularPrice, salePrice
...
//     min: [0, 'Stock cannot be negative']
//   }
// }, { _id: false }); // Prevent separate _id for each variant
...
//   },
//   images: {
//     type: [String], // Only store 4 images per product
//     required: true,
//     validate: {
//       validator: function(arr) {
...
// // Pre-save: Automatically set status to 'Out of stock' if all variants have stock 0
// productSchema.pre('save', function(next) {
//   const totalStock = this.variants.reduce((sum, v) => sum + v.stock, 0);
//   if(totalStock === 0) this.status = 'Out of stock';
//   else if(this.status === 'Out of stock') this.status = 'Available';
//   next();
// });
...
const productSchema = new Schema({
  productName: {
    ...
    minlength: [2, 'Product name must be at least 2 characters'],
    maxlength: [100, 'Product name cannot exceed 100 characters']
    // unique index will be created below, no need here
  },
  description: {
    type: String,
