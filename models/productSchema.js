...
productSchema.virtual("lowestPrice").get(function () {
  return this.variants?.length
    ? Math.min(...this.variants.map((v) => v.salePrice))
    : 0;
});

productSchema.virtual("highestPrice").get(function () {
  return this.variants?.length
    ? Math.max(...this.variants.map((v) => v.regularPrice))
    : 0;
});
...
productSchema.pre("save", function (next) {
  try {
    ...
    if (totalStock === 0 && this.status !== "Discontinue")
      this.status = "Out of stock";
    else if (totalStock > 0 && this.status === "Out of stock")
      this.status = "Available";

    if (this.variants?.length > 1)
      this.variants.sort((a, b) => a.size - b.size);
...
productSchema.statics.findAvailable = function () {
  return this.find({ status: "Available", isBlocked: false }).populate(
    "brand category"
  );
};
